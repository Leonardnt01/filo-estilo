...
const navItems = [
  ...
  { href: "/admin/promotions", label: "Promociones", icon: "M9.5 2a1.5 1.5 0 00-1.5 1.5v7a1.5 1.5 0 00.44 1.06l6 6a1.5 1.5 0 002.12 0l5-5a1.5 1.5 0 000-2.12l-6-6A1.5 1.5 0 0014.5 2h-5zM11.5 6a1.5 1.5 0 100-3 1.5 1.5 0 000 3z" },
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const SidebarContent = ({ forceFullWidth = false }: { forceFullWidth?: boolean }) => {
    const collapsed = isSidebarCollapsed && !forceFullWidth;
    return (
      <div className="flex h-full flex-col justify-between py-6">
        <div className={`space-y-6 ${collapsed ? "px-2" : "px-6"}`}>
          {/* Logo and Brand Title */}
          <div className={`flex ${collapsed ? "flex-col items-center gap-4 w-full" : "flex-row items-center justify-between gap-3"}`}>
            <Link
              href="/admin"
              onClick={() => setSidebarOpen(false)}
              className={`flex items-center justify-center group min-w-0 ${
                collapsed ? "w-12 h-12 self-center shrink-0" : ""
              }`}
            >
              ...
              {!collapsed && (
                <span className="text-xl font-bold tracking-tight transition-colors truncate ml-2" style={{ fontFamily: "var(--font-playfair), serif", color: "var(--text-primary)" }}>
                  Filo <span className="text-[var(--accent)]">Estilo</span>
                </span>
              )}
            </Link>
            {!collapsed && (
              <span className="rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider shrink-0" style={{ background: "var(--accent-soft)", color: "var(--accent)", border: "1px solid var(--accent-border)" }}>
                Admin
              </span>
            )}
            ...
            {!forceFullWidth && (
              <button
                ...
                className={`hidden lg:flex rounded-xl border border-[var(--border-strong)] bg-[var(--bg-surface)] text-[var(--text-muted)] hover:text-[var(--accent)] cursor-pointer hover:scale-105 active:scale-95 transition-all shrink-0 ${
                  collapsed ? "w-12 h-12 items-center justify-center self-center" : "p-1.5"
                }`}
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3.5 rounded-2xl px-4.5 py-3.5 text-sm font-semibold transition-all duration-300 ${
                    collapsed ? "justify-center px-0 w-12 h-12 self-center shrink-0" : ""
                  } ${
                    active
                ...
                >
                  <svg className="h-4.5 w-4.5 transition-transform duration-300 group-hover:scale-110 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.2">
        ...
        <div className={`space-y-4 pt-4 border-t border-[var(--border)] ${collapsed ? "px-2 pb-16 text-center flex flex-col items-center" : "px-6"}`}>
          <div className={`flex items-center ${collapsed ? "justify-center" : "justify-between"} w-full`}>
            {!collapsed && <span className="text-xs text-[var(--text-muted)] font-medium">Modo visual</span>}
  ...
  return (
    <div className="min-h-screen flex" style={{ background: "var(--bg-primary)" }}>
      ...
      <aside 
        className={`hidden lg:flex lg:flex-col lg:fixed lg:inset-y-0 z-40 border-r transition-all duration-300 ${
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 lg:hidden flex">
          ...
          >
            ...
            <SidebarContent forceFullWidth={true} />
          </aside>
        </div>
      )}
