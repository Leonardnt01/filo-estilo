...
export default function AdminServicesPage() {
  ...
  async function deactivate(id: string) {
    ...
    if (!res.ok) { const json = await res.json().catch(() => ({})); setError(json.error ?? "No se pudo desactivar servicio"); return; }
    setSuccess("Servicio desactivado"); await load();
  }
                ...
                <td>
                  {editingPriceId === item.id ? (
                    ...
                    <div className="flex gap-1.5">
                      ...
                      <button className="admin-btn admin-btn-danger" onClick={() => deactivate(item.id)}>
