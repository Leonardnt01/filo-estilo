...
export async function POST(request: Request) {
  ...
  const duplicateStatuses = ["active", "offered"];
  ...
  if (parsed.data.barber_id) {
    duplicateQuery = duplicateQuery.eq("barber_id", parsed.data.barber_id);
  } else {
    duplicateQuery = duplicateQuery.is("barber_id", null);
  }

  const { data: duplicateEntry, error: duplicateError } = await duplicateQuery.limit(1).maybeSingle();
  if (duplicateError) {
    return NextResponse.json({ error: duplicateError.message }, { status: 500 });
  }
  if (duplicateEntry) {
    return NextResponse.json(
      { error: "Ya tienes una solicitud activa de lista de espera para ese servicio y fecha." },
      { status: 409 },
    );
  }
  ...
  for (const select of WAITLIST_SELECT_VARIANTS) {
    ...
    if (!error) {
      return NextResponse.json(
        { ok: true, item: normalizeWaitlistEntryForClient(data as unknown as Record<string, unknown>) },
        { status: 201 },
      );
    }
  }
